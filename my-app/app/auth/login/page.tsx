...
export default function LoginPage() {
  ...
  // Sinkronkan login Google ke AuthContext + localStorage
  useEffect(() => {
    if (status === "authenticated" && session) {
      // akses token backend yg kamu set di callback session NextAuth
      // @ts-ignore
      const token = session.accessToken as string | undefined;
      if (!token) return;

      // Data minimal untuk AuthContext agar nama & email terisi
      const googleUser = {
      ...
      // login() sudah meng-handle penyimpanan ke localStorage dan state
